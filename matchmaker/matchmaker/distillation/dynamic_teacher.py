...
class DynamicTeacher():
    ...
    def __init__(self,
        ...
        self.cuda_device = torch.cuda.device_count() - 1
        #[torch.cuda.device_count() - 2,torch.cuda.device_count() - 1]
        self.logger = logger
    ...
    def __iter__(self) -> Iterator[TensorDict]:
        ...
        if self.num_workers==0:
            for batch in self.dynamic_teacher_one_process():
                yield batch

        else:
            ctx = mp.get_context("spawn") # need spawn here, otherwise CUDA fails
            queue: mp.JoinableQueue = ctx.JoinableQueue(30)
            worker = ctx.Process(
    ...
    def dynamic_teacher_one_process(self):
        console = Console()
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=use_fp16):
            for orig_batch in self.wrapped_dataloader:
                ...
                if use_multi:
                    output_pos, output_neg = parallel_apply(replicas, [pos_in, neg_in],
                    ...
                    output_neg, query_vecs_neg, doc_vecs_neg = output_neg
                    # colbert model
                    ib_output_pos = model.forward_inbatch_aggregation(query_vecs_pos,
                                                                      batch["query_tokens"]["attention_mask"],
                                                                      doc_vecs_pos,
                                                                      batch["doc_pos_tokens"]["attention_mask"])
                    ib_output_neg = model.forward_inbatch_aggregation(query_vecs_neg,
                                                                      batch["query_tokens"]["attention_mask"],
                                                                      doc_vecs_neg,
                                                                      batch["doc_neg_tokens"]["attention_mask"])
                    orig_batch["dyn_teacher_scores_pos"] = ib_output_pos.cpu()
                    orig_batch["dyn_teacher_scores_neg"] = ib_output_neg.cpu()
