    ...
    try:
        ...
        if from_scratch:
            for epoch in range(0, int(config["epochs"])):
                ...
                label_neg = torch.zeros(training_batch_size).cuda(cuda_device)
                    ...
                    for batch in input_loader:
                        ...
                        try:
                            ...
                            if ranking_score_pos.shape[0] != training_batch_size:                              # the last batches might (will) be smaller
                                label = torch.ones(ranking_score_pos.shape[0],device=ranking_score_pos.device)
                                label_neg = torch.zeros(ranking_score_pos.shape[0],device=ranking_score_pos.device)# but it should only affect the last n batches
                            ...
                            with torch.cuda.amp.autocast(enabled=use_fp16):
                             ...
                             if use_list_loss:
                                ...
                                #loss = ranking_loss_fn(ranking_score_pos, ranking_score_neg, label, label_neg)
